...
- `POST /api/admin/broadcast/forwarding-howto`, `POST /api/admin/broadcast/feedback-announcement`, `POST /api/admin/broadcast/teams-announcement` — admin-only one-off enhancement announcements (each supports `testTo`/`dryRun`); respect `unsubscribedAll`. Triggered by an authed admin POST (no UI); standard flow is dryRun → testTo self → real send
