...
- `POST /api/admin/broadcast/forwarding-howto`, `POST /api/admin/broadcast/feedback-announcement`, `POST /api/admin/broadcast/teams-announcement` — admin-only one-off enhancement announcements (each supports `testTo`/`dryRun`); respect `unsubscribedAll`. Triggered by an authed admin POST (no UI); standard flow is dryRun → testTo self → real send. **Superseded for future announcements** by `POST /api/admin/announcements` (persists an `Announcement` row + optional bell fan-out via `recordNotification` type "announcement" + optional pref-filtered email through the generic `announcementEmail` template) · `DELETE /api/admin/announcements/[id]`
