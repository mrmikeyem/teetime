...
import { forwardingHowToEmail } from "@/lib/email-templates";
...
/**
 * One-off enhancement announcement: how to use email-to-tee-time.
 *
...
export async function POST(req: Request) {
  ...
  const results = await Promise.allSettled(
    targets.map(async (r) => {
      ...
      const { subject, text, html } = forwardingHowToEmail({
        name: r.name,
        appUrl: APP_URL,
