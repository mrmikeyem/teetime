...
import { feedbackAnnouncementEmail } from "@/lib/email-templates";
...
/**
 * One-off enhancement announcement: the in-app feedback button.
 *
...
export async function POST(req: Request) {
  ...
  const results = await Promise.allSettled(
    targets.map(async (r) => {
      ...
      const { subject, text, html } = feedbackAnnouncementEmail({
        name: r.name,
        appUrl: APP_URL,
